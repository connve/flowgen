//! Response registry for correlating MCP requests with pipeline results.
//!
//! When an MCP `tools/call` request arrives, the server creates a registry entry
//! with channels for progress streaming and final result delivery. The entry's
//! correlation_id is threaded through `Event.meta` so the final task can look up
//! the channels and deliver the result back to the waiting SSE stream.
...
/// Progress event streamed to the MCP client during tool execution.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// Channels for delivering results back to the MCP SSE stream.
pub struct ResponseSender {
    /// For streaming intermediate progress events back to the MCP client.
    pub progress_tx: mpsc::Sender<McpProgressEvent>,
    /// For sending the final tool result. Wrapped in Option so it can be taken once.
    pub result_tx: Option<oneshot::Sender<McpToolResult>>,
}

/// Registry mapping correlation IDs to response channels.
///
/// Thread-safe registry that the MCP server endpoint writes to (creating entries)
/// and pipeline tasks read from (sending results). Entries are cleaned up after
/// the result is sent or on timeout.
#[derive(Clone)]
...
impl ResponseRegistry {
    ...
    ///
    /// Called by the MCP server endpoint when a `tools/call` request arrives.
    ...
    ///
    /// Called by the final pipeline task to retrieve the channels for result delivery.
