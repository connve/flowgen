...
mod tests {
    ...
    impl Dispatcher for TestDispatcher {
        type Registration = TestReg;
        type Extras = ();

        fn build_router(_state: DispatchState<Self::Registration, Self::Extras>) -> Router {
            Router::new()
        }
    }
    ...
    struct TestDispatcherWithExtras;

    impl Dispatcher for TestDispatcherWithExtras {
        type Registration = TestReg;
        type Extras = TestExtras;
    ...
    #[test]
    fn new_creates_server_with_empty_table() {
        let server = HttpServer::<TestDispatcher>::new("/test".to_string());
        assert_eq!(server.path(), "/test");
        assert_eq!(server.table.len(), 0);
    }

    #[test]
    fn new_with_extras_stores_extras() {
        let extras = TestExtras { value: 42 };
        let server =
            HttpServer::<TestDispatcherWithExtras>::new_with_extras("/ext".to_string(), extras);
        assert_eq!(server.extras().value, 42);
        assert_eq!(server.path(), "/ext");
        assert_eq!(server.table.len(), 0);
    }

    #[test]
    fn register_inserts_entry() {
        let server = HttpServer::<TestDispatcher>::new("/hook".to_string());
        server.register(
            "key1".to_string(),
            TestReg {
                flow: "flow_a".to_string(),
            },
        );
        assert_eq!(server.table.len(), 1);
        assert_eq!(server.table.get("key1").unwrap().flow_name(), "flow_a");
    }

    #[test]
    fn try_register_succeeds_on_new_key() {
        let server = HttpServer::<TestDispatcher>::new("/hook".to_string());
        let result = server.try_register(
            "key1".to_string(),
            TestReg {
                flow: "flow_a".to_string(),
            },
    ...
    #[test]
    fn try_register_returns_err_on_collision() {
        let server = HttpServer::<TestDispatcher>::new("/hook".to_string());
        server.register(
            "key1".to_string(),
            TestReg {
    ...
    #[test]
    fn deregister_flow_removes_all_entries_for_flow() {
        let server = HttpServer::<TestDispatcher>::new("/hook".to_string());
        server.register(
            "key1".to_string(),
            TestReg {
        ...
        assert_eq!(server.table.len(), 1);
        assert!(server.table.get("key1").is_none());
        assert!(server.table.get("key2").is_none());
    }

    #[test]
    fn deregister_flow_leaves_other_flows_intact() {
        let server = HttpServer::<TestDispatcher>::new("/hook".to_string());
        server.register(
            "key1".to_string(),
            TestReg {
        ...
        server.deregister_flow("flow_a");
        assert_eq!(server.table.len(), 1);
        assert_eq!(server.table.get("key2").unwrap().flow_name(), "flow_b");
    }

    #[test]
    fn with_credentials_path_sets_path() {
        let path = std::path::PathBuf::from("/etc/creds.json");
        let server = HttpServer::<TestDispatcher>::new("/hook".to_string())
            .with_credentials_path(Some(path.clone()));
        assert_eq!(server.credentials_path(), Some(path.as_path()));
    }

    #[test]
    fn with_credentials_path_none() {
        let server =
            HttpServer::<TestDispatcher>::new("/hook".to_string()).with_credentials_path(None);
        assert_eq!(server.credentials_path(), None);
    }

    #[test]
    fn with_auth_provider_sets_provider() {
        let server =
            HttpServer::<TestDispatcher>::new("/hook".to_string()).with_auth_provider(None);
        assert!(server.auth_provider().is_none());
    }

    #[test]
    fn path_returns_configured_path() {
        let server = HttpServer::<TestDispatcher>::new("/my/prefix".to_string());
        assert_eq!(server.path(), "/my/prefix");
    }
    ...
    fn clone_shares_table() {
        ...
        assert_eq!(cloned.table.len(), 1);

        // Mutations on clone are visible on original (shared Arc<DashMap>).
        cloned.register(
            "key2".to_string(),
            TestReg {
                flow: "flow_b".to_string(),
